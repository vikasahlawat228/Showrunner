...
| **Agentic Chat Co-Pilot**              | Persistent sidebar chat agent that can control all 10 agents, run/stop pipelines, create/edit buckets, and manage sessions via natural language. Supports `@-mention` for context injection, `/compact` for context management, and Plan/Execute modes for complex tasks. | As a Discovery Writer, I want to talk to my tool instead of navigating menus, so I can stay in flow while worldbuilding and writing. | Given I type "create a character called Zara" in the chat sidebar, When the agent processes my request, Then a Character bucket is created, shown in an artifact preview, and linked to the KG. |   🆕   |    P0    |
| **Multi-Session Management**           | Free-form named sessions with resume, context compaction, and layered memory (persistent project memory + ephemeral session context + on-demand retrieval).                                                                                                               | As a writer working across multiple days, I want to resume exactly where I left off with full context.                               | Given I type `/resume`, When I select a previous session, Then the chat loads my session digest, project memory, and noted next steps within 30 seconds.                                        |   🆕   |    P0    |
| **Artifact Preview Panel**             | Split-pane preview alongside chat showing structured content (prose, outlines, schemas, panels, research) generated by the agent. Supports accept/reject and inline editing.                                                                                              | As a writer, I want to see AI-generated outlines and prose previews without them cluttering the chat thread.                         | Given the agent generates a chapter outline, When it appears, Then the artifact preview panel shows the structured outline with approve/edit/dismiss actions.                                   |   🆕   |    P1    |
...
- **Agentic Chat Sidebar (global):** Persistent collapsible panel on the right side of every page. The writer's conversational interface to all 10 agents. Features: `@-mention` autocomplete for buckets/entities, `/commands` for session control (`/compact`, `/resume`, `/new-session`, `/end`, `/plan`), artifact preview panel for generated content, action blocks showing agent operations, token usage indicator, and session picker. The sidebar is context-aware — it auto-loads relevant entities based on the current page (e.g., scene characters when in Zen Mode). Visual operations (storyboard drag-drop, timeline visualization, KG minimap) remain GUI-only; the agent can update them but users manipulate them directly.
...
- **Chat:** `POST /api/v1/chat/message`, `GET /stream` (SSE), `POST /sessions`, `GET /sessions`, `GET /sessions/{id}`, `POST /sessions/{id}/compact`, `POST /sessions/{id}/end`, `GET /memory`, `PUT /memory`, `POST /plan`, `POST /plan/{id}/execute`, `POST /plan/{id}/modify`
...
| **Chat Coverage**           | 90%+ of non-visual operations can be performed entirely from the Agentic Chat Sidebar without page navigation.                    |
| **Session Continuity**      | Writer resumes a previous session and is oriented within 30 seconds via session digest + project memory.                          |
| **Context Efficiency**      | `/compact` preserves all story-critical context (decisions, entity references, pending items) while reducing token usage by 80%+. |
...
- **Phase J: Agentic Chat Co-Pilot** 🆕 (Planned — see `docs/AGENTIC_CHAT_CUJS.md` for detailed CUJs)
  - **Track 1 — Chat Foundation**: `ChatSidebar.tsx` collapsible panel on all pages. `ChatInput.tsx` with @-mention autocomplete + `/commands`. `ChatMessageList.tsx` with user/agent messages. `chatStore.ts` Zustand slice. Backend `chat.py` router with SSE streaming. `ChatSessionService` for session CRUD + YAML persistence.
  - **Track 2 — Context & Memory Layer**: Three-layer context model: Project Memory (persistent `project_memory.yaml`), Session Context (ephemeral, compactable), On-Demand Retrieval (ChromaDB + KG). `ChatContextManager` for token tracking + compaction. `ProjectMemoryService` for Layer 1 CRUD. `/compact` command with LLM summarization. Token usage indicator in sidebar header.
  - **Track 3 — Agent Tool Belt**: Wire all 10 agents as chat-invocable tools: Write, Brainstorm, Outline, Research, Continuity, Style, Translate, Storyboard, Pipeline, Schema. `QuickCaptureAgent` for NL parsing → structured bucket creation. @-mention resolution for deterministic context injection (`@character`, `@scene`, `@research`, etc.). `ActionBlock.tsx` for transparent agent action display.
  - **Track 4 — Artifact Preview & Interaction**: `ArtifactPreview.tsx` split-pane panel for structured content display (prose, outlines, schemas, panels, research). Accept/reject/edit actions on artifacts. Diff view for revisions. Streaming preview for real-time content generation.
  - **Track 5 — Session Management**: `SessionPicker.tsx` with resume/create/delete. Named sessions with auto-generated digests on end. Session history with "next steps" tracking. `/new-session`, `/resume`, `/end` commands. Session digests persisted to `session_log`.
  - **Track 6 — Plan/Execute Mode**: `PlanViewer.tsx` for multi-step plan display with per-step approval. `PlanExecutionEngine` (lightweight, simpler than full Pipeline). `/plan` mode for complex tasks — agent proposes steps, writer approves, then `/execute`. Mid-plan modification support.
  - **Track 7 — Pipeline Control from Chat**: Chat-based pipeline launch/pause/resume/cancel. Inline approval gates (chat-based alternative to `PromptReviewModal`). Real-time pipeline status streaming to chat. Mid-execution model switching.

- **Phase K: Unified Data Access Layer** 🆕 (Planned)
  - `UnitOfWork` for atomic YAML + SQLite + Event writes
  - `MtimeCache` for read-path caching with file-mtime invalidation
  - `ProjectSnapshotFactory` for batch context loading
  - `ContextAssembler` — unified prompt assembly replacing `ContextCompiler` + `ContextEngine` duality

- **Phase L: IDE Integration & Story Versioning** 🆕 (Planned — see `docs/ROADMAP_PHASE_L.md`)
  - **Track 1 — Git for Stories**: Story-level `.gitignore` strategy, `showrunner git stage-story` / `commit-message` / `history` / `diff` CLI commands, `git_router.py` REST API for frontend Git panel, arc-based branching (`git checkout -b arc/redemption`)
  - **Track 2 — IDE ↔ UI Real-Time Sync**: `FileWatcherService` debounced YAML monitoring → KG sync → SSE `GRAPH_UPDATED` broadcast. Frontend `useProjectEvents.ts` → Zustand store auto-refresh.
  - **Track 3 — Cascade Updates**: `CascadeUpdateService` analyzes scene diffs via LLM → extracts character/location changes → auto-updates related YAML files. Available via CLI (`showrunner cascade update`) and REST API.
  - **Track 4 — Agent Skills Formalization**: Standard `SKILL.md` frontmatter (name, description, version, triggers, output_format, required_context). Create missing `writing_agent.md`. Add 5 new skills: `session_manager`, `character_updater`, `git_story_ops`, `project_initializer`, `continuity_reviewer`.
  - **Track 5 — Workflow Files**: `.agents/workflows/` directory with multi-step automated flows: `write-scene.md`, `new-chapter.md`, `daily-session.md`, `arc-planning.md`.
  - **Track 6 — Frontend Wiring**: Wire stubbed ChatSlice functions to API. Build Git Panel component. Add cascade update trigger in Zen Mode. SSE-driven store refresh on `GRAPH_UPDATED`.
  - **Track 7 — Cloud Sync**: Optional Google Drive backup via `CloudSyncService`. Push/pull endpoints on `sync_router.py`.
...
- **Chat Context Window Exhaustion:** Long creative sessions with the Agentic Chat will fill the context window, degrading agent quality.
  *Mitigation:* Three-layer context model with automatic compaction at 80% threshold. Project Memory (persistent) survives compaction. Session context is LLM-summarized into ~2K token digests. @-mentions pull fresh data on demand rather than keeping everything in context.
- **Agent Action Reversibility:** The chat agent can create/modify buckets, run pipelines, and alter story structure — mistakes could cascade.
  *Mitigation:* Progressive autonomy levels (Ask → Suggest → Execute). All mutations flow through EventService for full undo. Destructive actions require explicit confirmation. Plan Mode for complex multi-step tasks forces writer approval before execution.
- **Chat-GUI State Synchronization:** Edits made via chat must immediately reflect on visual pages (KG, Storyboard, Timeline) and vice versa.
  *Mitigation:* All mutations go through the existing `ContainerRepository` → SSE broadcast pipeline. Chat actions use the same service layer as GUI actions. FileWatcherService ensures bilateral sync.
...
| **Cursor / Claude Code** | Powerful agentic chat for code, but no domain knowledge of creative writing, story structure, or worldbuilding.                       | Adapts the agentic sidebar pattern (sessions, @-mentions, context compaction, plan/execute) specifically for creative workflows: bucket-aware context, story-scoped memory, pipeline control, and multi-agent orchestration — all through natural language. |
| **Novelcrafter**         | Codex auto-detection and chat are strong, but no pipeline system, no visual storyboard, no research agent, and limited model freedom. | Combines Novelcrafter's entity-aware chat with Showrunner's Glass Box pipeline system, 10-agent ecosystem, model cascade, and visual storyboard — all accessible from a single sidebar.                                                                     |
