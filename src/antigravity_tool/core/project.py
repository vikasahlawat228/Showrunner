...
from antigravity_tool.errors import ProjectError  # noqa: F401 (re-exported for backward compat)
...
from antigravity_tool.schemas.world import WorldSettings, Faction
...
MANIFEST_FILE = "antigravity.yaml"
CREATIVE_ROOM_MARKER = ".antigravity-secret"


class Project:
    """Represents an Antigravity project on disk.
    ...
    def find(cls, start: Path | None = None) -> Project:
        """Walk up from `start` (default cwd) to find the nearest antigravity.yaml."""
        ...
        raise ProjectError(
            f"No {MANIFEST_FILE} found. Run 'antigravity init' to create a project."
        )
