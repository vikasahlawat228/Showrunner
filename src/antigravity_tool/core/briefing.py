...
When Claude Code opens a new session in an Antigravity project, it reads
the CLAUDE.md. The `antigravity brief` command regenerates the dynamic
...
from antigravity_tool.core.workflow import WorkflowState, STEP_LABELS, WORKFLOW_STEPS
from antigravity_tool.core.session_manager import DecisionLog, SessionLog
from antigravity_tool.schemas.session import ChapterSummary, ProjectBrief
...
class BriefingGenerator:
    ...
    def _suggest_next(self, current_step: str, chapters: list[ChapterSummary]) -> tuple[str, str]:
        ...
        suggestions = {
            "world_building": ("Build the world settings", "antigravity world build"),
            "character_creation": ("Create characters", "antigravity character create \"Name\" --role protagonist"),
            "story_structure": ("Outline the story structure", "antigravity story outline"),
            "scene_writing": ("Write the next scene", "antigravity scene write --chapter 1 --scene 1"),
            "screenplay_writing": ("Convert scene to screenplay", "antigravity screenplay generate --chapter 1 --scene 1"),
            "panel_division": ("Divide screenplay into panels", "antigravity panel divide --chapter 1 --scene 1"),
            "image_prompt_generation": ("Generate image prompts", "antigravity prompt generate --chapter 1"),
        }
        action, cmd = suggestions.get(current_step, ("Check project status", "antigravity status"))
        return action, cmd
