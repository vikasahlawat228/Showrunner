"""antigravity character - character management commands."""
...
def show_dna(
    ...
    if not char.dna.face.face_shape:
        print_info(f"No DNA block for '{name}'. Run 'antigravity character generate-dna {name}'.")
        return
    console.print()
...
def list_characters() -> None:
    ...
    if not characters:
        print_info("No characters found. Run 'antigravity character create' first.")
        return
