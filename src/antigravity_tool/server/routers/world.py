...
from antigravity_tool.server.api_schemas import PromptResponse
from antigravity_tool.server.deps import get_world_service
...
async def get_world(svc: WorldService = Depends(get_world_service)):
    ...
    if not settings:
        return {"status": "empty", "message": "No world settings. Run 'antigravity world build'."}
