...
Resolution order:
  ...
  3. Per-Agent default (from showrunner.yaml model_overrides)
  4. Project default (from showrunner.yaml default_model)
"""
...
class ModelConfigRegistry:
    ...
    def _load_config(self) -> None:
        """Load model configuration from showrunner.yaml."""
        yaml_path = self._project_path / "showrunner.yaml"
        if not yaml_path.exists():
            logger.warning("No showrunner.yaml found at %s — using defaults", self._project_path)
            return
        ...
        try:
            ...
            logger.error("Failed to load model config from showrunner.yaml: %s", e)
    ...
    def resolve(
        ...
        Priority:
          ...
          3. Project-level agent override (showrunner.yaml model_overrides)
    ...
    def update_config(
        ...
        """Update project model configuration and persist to showrunner.yaml."""
