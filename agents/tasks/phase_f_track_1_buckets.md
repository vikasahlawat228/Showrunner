...
1. **Update `GenericContainer`**
   - Open `src/showrunner_tool/schemas/container.py`.
...
2. **Build Project Schemas**
   - Create `src/showrunner_tool/schemas/project.py`.
...
3. **Build the Projects Router**
   - Create `src/showrunner_tool/server/routers/projects.py`.
   - Implement `POST /api/v1/projects/` to generate a new multi-project directory layout on disk. It must create the subfolders: `containers/`, `schemas/`, `.showrunner/`, and initialize empty `event_log.db` and `knowledge_graph.db` databases.
