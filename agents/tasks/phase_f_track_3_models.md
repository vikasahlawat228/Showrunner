...
1. **Build `ModelConfig` Schemas**
   - Create `src/showrunner_tool/schemas/model_config.py`.
...
2. **Implement `ModelConfigRegistry`**
   - Create `src/showrunner_tool/services/model_config_registry.py`.
   - This service reads the `showrunner.yaml` file from the Workspace to seed project-level defaults.
...
3. **Update Dependency Injection (`deps.py`)**
   - Open `src/showrunner_tool/server/deps.py`.
...
4. **Build the Models Router**
   - Create a new FastAPI router: `src/showrunner_tool/server/routers/models.py`.
   ...
   - `PUT /api/v1/models/config`: Overwrite the user's `showrunner.yaml` model configurations.
