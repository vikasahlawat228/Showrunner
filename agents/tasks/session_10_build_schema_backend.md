...
- `ContainerSchema` and `FieldDefinition` Pydantic models in `src/antigravity_tool/schemas/container.py`
- `SchemaRepository` in `src/antigravity_tool/repositories/container_repo.py` that persists schemas as YAML files in `/schemas/`
...
### 2. Create Schema CRUD Router (new file: `src/antigravity_tool/server/routers/schemas_router.py`)
